...
const __1 = require("..");
describe("changeset", () => {
    const schema = new __1.Schema("comments")
        .field("author", __1.StringT, { default: "anonymous" })
        .field("color", __1.StringT, { default: () => "red" })
        .field("rating", __1.NumberT)
        .field("content", __1.StringT);
    ...
    describe("changeset", () => {
        test("new object", () => {
            const changeset = new __1.Changeset(schema, undefined);
        ...
        test("changes field on correct data", () => {
            const changeset = new __1.Changeset(schema, undefined, { content: "some content" }, ["content"]);
            expect(changeset.errors).toEqual([]);
        ...
        test("invalidates changeset on bad data", () => {
            const changeset = new __1.Changeset(schema, undefined, { rating: "definitely not a number" }, ["rating"]);
    ...
    describe("applyChanges", () => {
        test("new object", () => {
            const newComment = new __1.Changeset(schema, undefined).applyChanges();
        ...
        test("changes fields on new object", () => {
            const newComment = new __1.Changeset(schema, undefined, { content: "some content", rating: 1 }, ["content", "rating"]).applyChanges();
            expect(newComment).toEqual({
                author: "anonymous",
                color: "red",
        ...
        test("apply changes creates new object", () => {
            const newComment = new __1.Changeset(schema, comment, {}, []).applyChanges();
            expect(newComment).toEqual(comment);
            expect(newComment).not.toBe(comment);
        });
        test("input data is immutable", () => {
            const newComment = new __1.Changeset(schema, comment, { author: "new author" }, ["author"]).applyChanges();
            expect(newComment).not.toBe(comment);
        });
        test("ignores changes not listed in allowed", () => {
            const changed = new __1.Changeset(schema, comment, { author: "new author" }, []).applyChanges();
            expect(changed).toEqual(comment);
        });
        test("handles not provided params", () => {
            const newComment = new __1.Changeset(schema, comment, {}, [
                "author"
            ]).applyChanges();
            expect(newComment).toEqual(comment);
        });
        test("changes fields listed in allowed", () => {
            ...
            const newComment = new __1.Changeset(schema, comment, changes, [
                "author",
                "color",
                "rating",
                "content"
